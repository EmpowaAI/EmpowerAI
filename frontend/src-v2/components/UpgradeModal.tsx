...
export const UpgradeModal = ({ open, onOpenChange, feature }: Props) => {
  const { isAuthenticated, login, upgrade } = useAuth();

  const handleUpgrade = () => {
    if (!isAuthenticated) {
      // soft sign-in for the demo flow; real flow will route to backend checkout
      login("Demo User", "demo@empowa.org");
    }
    // Stub — your team plugs Paystack/Yoco checkout in here.
    upgrade();
    onOpenChange(false);
  };
      ...
      <DialogContent className="overflow-hidden p-0 sm:max-w-md">
        ...
        <div className="space-y-5 p-6">
          ...
          <div className="rounded-lg border border-border/70 bg-muted/40 p-3 text-[11px] text-muted-foreground">
            <strong className="text-primary">CV Analyser stays Mahala</strong> — always free,
            no card needed.
          </div>
          ...
          <div className="flex flex-col-reverse gap-2 sm:flex-row">
            ...
            <Button variant="cta" className="shimmer w-full flex-1" onClick={handleUpgrade}>
              Upgrade for R50/mo
