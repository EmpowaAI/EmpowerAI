  ...
  getDashboardStats: async () => {
    try {
      ...
      const [twinResponse, opportunitiesResponse] = await Promise.allSettled([
        twinAPI.get(),
        opportunitiesAPI.getAll(careerGoalsFilter ? { careerGoals: careerGoalsFilter, limit: 1 } : { limit: 1 }),
      ]);
...
export const adminAPI = {
  getStats: async (adminKey: string) => adminRequest<any>('/admin/stats', adminKey),
  getCareerTaxonomy: async (adminKey: string) => adminRequest<any>('/admin/career-taxonomy', adminKey),
