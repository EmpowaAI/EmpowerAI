...
export async function streamChat({
  ...
  try {
    ...
    // Sanitize focus value
    const focusValue = localStorage.getItem('careerFocus') || 'growth';
    ...
    // FIX: Extract the last user message as `message` string.
    // The backend validator requires message: string (the current input).
    // The backend chatWithTwin(userId, userMessage, messages) uses both.
...
function simulateChat(
  ...
  try {
    ...
    if (userMessages === 0) {
      ...
      const skills = ['Communication', 'Problem Solving'];
      ...
      response = `I've put together your profile. Your empowerment score reflects your potential in the South African job market.\n\n[COMPLETE]\n[PROFILE: ${JSON.stringify({
        name, careerStage, province, industry, education, skills, challenges, goals, empowermentScore: score
      })}]`;
    }
  ...
  if (profileMatch) {
    try {
      ...
      try {
        ...
        if (Object.keys(extracted).length > 0) profile = extracted;
      } catch (e2) {
