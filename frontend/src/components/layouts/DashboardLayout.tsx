...
export default function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  return (
    <>
      <div className="fixed inset-0 bg-[radial-gradient(ellipse_at_top_right,hsl(var(--primary)/0.08),transparent_60%)] pointer-events-none" />
      <div className="fixed inset-0 bg-[radial-gradient(ellipse_at_bottom_left,hsl(var(--accent)/0.05),transparent_60%)] pointer-events-none" />
        ...
        <div className="flex-1 flex flex-col min-w-0 h-full">
          ...
          <header className="sticky top-0 z-30 flex h-16 flex-shrink-0 items-center justify-between border-b border-border/60 bg-background/85 px-4 backdrop-blur-md sm:px-6">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={handleLogout}
                className="hidden lg:flex items-center gap-1.5 rounded-lg border border-border px-3 py-1.5 text-xs font-semibold text-muted-foreground hover:text-destructive hover:border-destructive/40 hover:bg-destructive/5 transition-colors"
                title="Sign Out"
                type="button"
              >
